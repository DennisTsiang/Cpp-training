...
namespace STM32F407 {
    ...
    // SFINAE aliases
    // We can define the type returned by std::enable_if as bool; then we can write
    // code as follows:
    // template <typename T, is_some_trait<T> = true> void func() { }
    //
    ...
    //
    template <typename U>
    using is_read_only = typename std::enable_if<std::is_base_of<read_only, U>::value, bool>::type;

    template <typename U>
    using is_write_only = typename std::enable_if<std::is_base_of<write_only, U>::value, bool>::type;

    template <typename U>
    using is_read_write = typename std::enable_if<std::is_base_of<read_write, U>::value, bool>::type;
    ...
    private:
        ...
        private:
            friend class Register<sz, Register_Ty>;     // The parent must be able to construct.
            Register<sz, Register_Ty>* owner;           // The Register object that created this proxy.
